<template>
  <view class="flex flex-col">
    <view class="flex flex-row items-center justify-between mb-[22rpx]">
      ...
      <text class="text-base leading-6 text-neutral"
        >提现余额 {{ withdrawInfo.tixinmoney }} {{ symbolStore.code }}</text
      >
    </view>
    ...
    >
      ...
      <template v-if="symbolStore.value == 'USDT'">
        <view class="flex flex-row items-center justify-between mt-[10rpx]">
          <text
            v-if="withdrawInfo.tixian_usdt_fee == 0"
            class="text-base leading-6 font-medium text-neutral-theme"
            >实际到账 {{ usdtAmount || 0 }} USDT</text
          >
        </view>
        <view
          v-if="withdrawInfo.tixian_usdt_fee > 0"
        ...
        >
          <view class="flex flex-row items-center">
            ...
            <text class="text-neutral-error mr-[20rpx]"
              >{{ withdrawInfo.tixian_usdt_fee || 0 }} USDT</text
            >
          </view>
          <view class="flex flex-row items-center">
      ...
      <template v-else-if="symbolStore.value == 'CNY'">
        <view class="flex flex-row items-center justify-between mt-[10rpx]">
          <text class="text-base leading-6 font-medium text-neutral-theme"
            >{{ currentRate }} CNY = 1 USDT</text
          >
          <text
            v-if="withdrawInfo.tixian_usdt_fee == 0"
            class="text-base leading-6 font-medium text-neutral-theme"
        ...
        >
          <view class="flex flex-row items-center">
            <text class="text-neutral mr-[10rpx]">提现手续费</text>
            <text class="text-neutral-theme mr-[20rpx]"
              >{{ withdrawInfo.tixian_usdt_fee || 0 }} USDT</text
            >
          </view>
          <view class="flex flex-row items-center">
    ...
    >
      ...
      <template v-if="symbolStore.value == 'USDT'">
        <view class="flex flex-row items-center justify-between mt-[10rpx]">
          ...
          >
          <text
            v-if="withdrawInfo.tixian_cny_fee == 0"
            class="text-base leading-6 font-medium text-neutral-theme"
            >实际到账 {{ cnyAmount || 0 }} CNY</text
          >
        </view>
        <!-- USDT转CNY有手续费情况 -->
        <view
    ...
    <view
      ...
      v-if="+tixianzhanghu"
    >
      <text class="text-base leading-6 font-medium"
        >提现到 <text class="text-neutral-theme">投资余额</text> 继续投资
