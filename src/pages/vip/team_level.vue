<template>
  <layout navTitle="团队等级" bgType="team-level" ref="layoutRef" :refresher="true" @onRefresh="handleRefresh">
    <!-- 动态背景层 -->
    <!-- <view 
      class="fixed inset-0">
    </view> -->
    <z-swiper ref="swiperRef" class="my-[16rpx] h-[320rpx] relative z-10" v-model="vip_icons" @slideChange="onChange"
      slidesPerView="auto" centeredSlides :spaceBetween="-30" grabCursor>
      <z-swiper-item v-for="(item, index) in vip_icons" :key="index" :custom-style="{ width: '320rpx' }">
        <view class="flex flex-col justify-end items-center relative">
          <image class="w-[296rpx] h-[296rpx] mt-[0rpx]" :class="[
            index === activeIndex
              ? 'opacity-100 scale-[1]'
    ...
    <view class="relative pb-[24rpx] box-border w-full z-10">
      <view class="rounded-[32rpx] box-border relative p-[32rpx] mx-[32rpx] flex flex-col justify-between"
        ...
        <view class="mb-[20rpx] flex flex-row w-full items-center justify-between">
          <text class="text-base text-neutral-white">您当前的等级</text>
          <view class="text-lg flex flex-row items-center text-neutral-white">
            <image :src="levelIcon" mode="widthFix" class="w-[48rpx] h-[48rpx] mr-[4rpx]" />
            {{ Currentlevel.agentname }}
          </view>
        </view>
        <view
          class="mb-[32rpx] bg-white/20 rounded-[8rpx] box-border p-[10rpx] flex flex-row w-full items-center justify-between tip-box">
          <view class="text-sm text-white/70">
            温馨提示：如果各项数据都显示已达标，等级却没有更新，请点击
            <text class="text-[#24E5FF]">“我的团队”</text>
            即可更新等级状态

          </view>
        </view>
        <view class="flex flex-row justify-between">
          <view @click="navigateTo(routes.promoRecord)"
      ...
      <view class="p-[32rpx] mt-[32rpx]">
        <view class="text-lg font-medium flex flex-row items-center my-[48rpx] mt-[16rpx] justify-center">
          <image class="w-[24rpx] h-[24rpx]" src="/static/images/user/hy_level_black.svg" mode="widthFix" />
          <text class="mx-2 text-xl bg-gradient-blue bg-clip-text text-transparent"> 升级评估 </text>
          <image class="w-[24rpx] h-[24rpx]" src="/static/images/user/hy_level_black.svg" mode="widthFix" />
        </view>
        <view class="flex-col fb-align-center">
          <view v-if="list.length && lastteams === list[list.length - 1]['id']"
          ...
          <view class="flex flex-col w-full">
            <template v-for="item in teamLevelItems" :key="item.title">
              <teamLevelItem v-if="config[item.configKey] == 1"
                :title="item.titleKey ? nextlevel[item.titleKey] : item.title" :unit="item.unit"
                :levelReq="nextlevel[item.levelReqKey]" :levelGap="nextlevel[item.levelReqKey] > Currentlevel[item.currentValueKey]
