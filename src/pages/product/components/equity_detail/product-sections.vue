<template>
  <wk-stroke-bg class="mt-[24rpx]" v-if="hasContent">
    ...
    <template v-for="section in sections" :key="section.title">
      ...
      >
        <view class="flex flex-row items-center text-base text-neutral-theme leading-6">
        ...
        <view class="flex flex-col ml-[10rpx] w-full flex-1 flex-wrap">
          <view v-for="(val, index) in data[section.key]" :key="val" class="flex flex-row items-start">
            <text v-if="!section.noIndex" :class="[
                'text-base font-medium mr-[10rpx] leading-6',
                'text-neutral',
              ]">{{ index + 1 }}. </text>
            <text
              :class="[
            ...
            >
            <!-- <text
              v-if="val != data[section.key][data[section.key].length - 1]"
              :class="['text-base mx-[2rpx] font-medium', 'text-neutral-theme']"
              >&</text
            > -->
          </view>
        </view>
...
const sections = [
  // { title: "投资注意", key: "attention_tag_list", color: "text-neutral-warning" },
  //{ title: "投资要求", key: "demand_tag_list" },
  { title: "特殊福利", key: "special_tag_list" },
  { title: "支持使用", key: "supportuse" },
  { title: "认购赠送", key: "investgift" },
  { title: "推广奖励", key: "allreward" },
  // { title: "补充说明", key: "jiangli_remark", color: "text-neutral-warning", noIndex: true },
