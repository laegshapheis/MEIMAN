<template>
  <view
    ...
    <image src="/static/images/profile/account_bg.png" mode="widthFix"
      class="w-[324rpx] h-[274rpx] absolute top-0 right-0"></image>
    <!-- 账户信息 -->
    <view>
      <view class="px-[32rpx] py-[24rpx]">
        <view class="flex flex-row items-center justify-between w-full">
          ...
          <view @click="handleShowData">
            <uv-icon v-if="showData" name="eye" color="#fff" size="38rpx"></uv-icon>
            <uv-icon v-else name="eye-off-outline" color="#fff" size="38rpx"></uv-icon>
          </view>
        </view>
      </view>
      ...
      <view>
        <view v-if="openCards > 0"
          ...
          <view class="flex flex-row justify-center items-center">
            <text>实时汇率</text>
            <text class="ml-[16rpx]">1 {{ symbolStore.exchangeUsdtLabel }} = {{ currentRate }} CNY</text>
            <image @click="handleSwitchRate" src="/static/images/profile/switch.svg" mode="widthFix"
              class="ml-[16rpx] w-[24rpx] h-[24rpx]"></image>
          </view>
        </view>
        ...
        <view class="relative">
          <view class="gap-[8rpx] break-all">
            <view class="grid grid-flow-row-dense grid-cols-2 ">
              <!-- 投资余额 -->
              <view class=" flex flex-col py-[24rpx] min-w-[250rpx] box-border pl-[32rpx]">
                <view class="flex flex-row items-center" @click="$emit('onTips', 0)">
                  <text class="text-sm text-white/60 mb-[4rpx]">可用余额({{ symbol }})</text>
                  <text class="ml-[5rpx]"><uv-icon size="24rpx" color="rgba(255, 255, 255, 0.60)"
                      name="question-circle"></uv-icon></text>
                </view>
              ...
              <!-- 提现余额 -->
              <view class="pl-[32rpx] flex flex-col py-[24rpx] min-w-[250rpx] box-border">
                <view class="flex flex-row items-center" @click="$emit('onTips', 1)">
            ...
            <view class="grid grid-flow-row-dense grid-cols-2">
              <!-- 累计收益 -->
              <view class="pl-[32rpx] flex flex-col py-[24rpx] min-w-[250rpx] box-border">
                <view class="flex flex-row items-center" @click="$emit('onTips', 4)">
                  <text class="text-sm text-white/60 mb-[4rpx]">累计收益({{ symbol }})</text>
                  <text class="text-sm ml-[5rpx]"><uv-icon size="24rpx" color="rgba(255, 255, 255, 0.60)"
                      name="question-circle"></uv-icon></text>
                </view>
                <view class="text-2xl font-semibold text-white">
              ...
              <!-- 任务质押 -->
              <view class="pl-[32rpx] flex flex-col py-[24rpx] min-w-[250rpx] box-border" v-if="userInfo.lockmoney > 0">
                <view class="flex flex-row items-center" @click="$emit('onTips', 999)">
              ...
              <!-- 我的积分 -->
              <view v-else class="pl-[32rpx] flex flex-col py-[24rpx] min-w-[250rpx] box-border">
                <view class="flex flex-row items-center" @click="$emit('onTips', 5)">
    ...
    <view class="rounded-[24rpx] rounded-bottom-[0rpx] grid grid-cols-2 gap-[16rpx] p-[24rpx] bg-white">
      <template v-for="item in menus" :key="item.icon_url">
        <view @click="$emit('onNavTo', item)" class="flex flex-row justify-center items-center"
          v-if="!item.switch || (item.switch && userInfo[item.switch])">
          <image class="w-[40rpx] h-[40rpx] relative z-0" :src="item.icon_url" mode="widthFix"></image>
          <text class="text-neutral-theme font-medium relative z-1 text-base ml-[16rpx]">{{ item.title }}</text>
        </view>
      </template>
    </view>
  </view>
