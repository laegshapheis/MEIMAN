<template>
    ...
    >
      <template v-slot:navRight>
        <view
          class="flex flex-row w-full items-center justify-end bg-black/10 rounded-full p-[12rpx]"
      ...
      >
        <!-- start 顶部汇总信息 -->
        <view
          class="flex flex-row justify-start mx-auto w-full h-[112rpx] rounded-[24rpx] bg-card3"
        >
          <view
            class="flex flex-row items-center justify-center w-[100%] px-[28rpx] py-[41rpx] gap-[10rpx]"
          >
            <image
              class="w-[48rpx] h-[48rpx]"
              src="/static/images/profile/transfer_card_icon.png"
              mode="widthFix"
            ></image>
            <text class="font-medium text-lg text-white"
              >我的收款账号: {{ userInfo.invicode }}</text
            >
            <view
              class="grow flex flex-row justify-end text-white text-base font-medium"
              @click="copyCode"
            >
              <image
                src="/static/images/profile/tranfer_copy.png"
                mode="widthFix"
                class="w-[32rpx] h-[32rpx]"
              ></image>
            </view>
        ...
        <view class="flex flex-col mt-[64rpx]">
          <view class="flex flex-row items-center justify-between mb-[22rpx]">
            ...
            <text class="text-base leading-6 text-neutral" @click="openContact"
              >历史收款人</text
            >
          </view>
          ...
          <view
            class="flex flex-row items-center justify-center h-[120rpx] mt-[32rpx] bg-neutral-inputBg rounded-[24rpx]"
          >
            <uv-input
              class="h-[120rpx]"
              v-model="params.zhuanuser"
              type="number"
              maxlength="8"
              placeholder="请输入会员的收款账号"
              border="0"
              :placeholderStyle="{
                color: $colors.secondary,
                fontSize: '32rpx',
          ...
          <view
            class="flex flex-row items-center justify-between mt-[66rpx] mb-[22rpx]"
          >
          ...
          >
            <wk-picker
              style="width: 100%"
              @change="handleChangeTransferInMethod"
              rangeKey="title"
            ...
            >
              <view class="flex flex-row justify-between px-[30rpx]">
                <text
                  ...
                  v-if="currentInMethodIndex == -1"
                  >请选择接收方式</text
                >
                <text
                  class="text-lg font-bold"
                  v-if="currentInMethodIndex >= 0"
          ...
          >
            <wk-picker
              class="dkh_btn"
              style="width: 100%"
              @change="handleChangeTransferOutMethod"
              rangeKey="title"
            ...
            >
              <view class="flex flex-row justify-between px-[30rpx]">
                <text
                  class="text-lg text-neutral-secondary"
                  v-if="currentOutMethodIndex == -1"
                  >请选择转出方式</text
                >
                <text
                  class="text-lg font-bold"
                  v-if="currentOutMethodIndex >= 0"
          ...
          <view
            class="flex flex-row items-center justify-between mt-[22rpx]"
            v-if="+userInfo.tixianzhanghu"
          >
            <text class="text-base leading-6"
              >从 <text class="text-neutral-error">提现余额</text> 转出
              <text class="text-neutral-error"
                >奖励{{ userInfo.tixianzhanghu }}%</text
              ></text
            >
          </view>
          ...
          <view class="flex flex-row justify-between mt-[66rpx] mb-[22rpx]">
            ...
            <text class="text-2xs leading-6 font-medium text-neutral-regular"
              >投资余额 {{ userInfo.amount }} {{ symbolStore.code }}，提现余额
              {{ userInfo.txmoney }} {{ symbolStore.code }}</text
            >
          </view>
  
          <wk-stroke-bg size="small" class="flex flex-col">
            <view class="flex flex-row items-center">
              <text class="text-xl text-neutral">{{ symbolStore.symbol }}</text>
              <uv-input
                v-model="params.zhuanmoney"
                type="digit"
                maxlength="7"
                placeholder="请输入转账金额(对方到账金额)"
                border="0"
                :placeholderStyle="{
                  color: $colors.secondary,
                  fontSize: '32rpx',
