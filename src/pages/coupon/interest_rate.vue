<template>
  <layout ref="layoutRef" :refresher="true" @onRefresh="handleRefresh" navTitle="加息券" bgType="bg-main2"
    :isLottie="false" @reachBottom="getPaginationList">
    <view class="px-page-x pb-[30rpx] box-border w-full flex flex-col">
      <wk-tabs :list="tabList" :current="type" mode="pill" :isButton="true" @change="changeType" :scrollable="false" />
      ...
      <view class="flex flex-col mt-[24rpx]">
        <view v-for="item in dataList" :key="item.id"
          ...
          <view class="w-full h-[1px] bg-transparent mb-[16rpx]"></view>
      ...
      <Empty class="mt-[300rpx]" title="暂无加息券" v-if="dataList.length == 0" />
    </view>
...
const cacheData = cacheManager.getCache("rateCoupon");
const page = ref(1);
const dataList = ref([]);
