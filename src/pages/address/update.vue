<template>
  <layout :navTitle="form.id ? '编辑收货地址' : '新增收货地址'" bgType="bg-address" :isLottie="false">
    <view class="px-[24rpx] py-[0rpx] mt-[24rpx] rounded-[32rpx]">
      <view
        class="w-full box-border px-[32rpx] flex flex-col justify-between bg-black rounded-[32rpx] shadow-[0_0_16px_0_#266AFF_inset]">
        <uv-form labelWidth="140rpx" :labelStyle="{ color: $colors.DEFAULT, fontSize: '30rpx' }">
          <uv-form-item v-for="item in formConfig.fields" :label="item.label" @click="() => item.func && item.func()"
            :key="item.name">
            <uv-textarea v-if="item.type === 'textArea'" v-model="form[item.name]" :placeholder="item.placeholder"
              border="none" height="88" :textStyle="{ color: $colors.DEFAULT, fontSize: '30rpx', fontWeight: 400 }"
              :autoHeight="true" :customStyle="{ background: 'transparent', padding: '0 10rpx' }"
              :placeholderStyle="placeholderStyle" v-bind="item">
            </uv-textarea>
            <uv-input v-else v-model="form[item.name]" border="none" :color="$colors.DEFAULT" fontSize="30rpx"
              :customStyle="{ fontWeight: 400 }" :placeholder="item.placeholder" :placeholderStyle="placeholderStyle"
              @input="(value) => handleInput(value, item)" v-bind="item">
              <template v-slot:prefix>
                <wk-country-code-picker v-if="item.inputType === 'tel'" ref="countryCodePicker" :flagIcon="item.icon"
                  :defaultCountry="{
                    name: '',
                    code: countryCode
            ...
            <template v-slot:right v-if="item.rightIcon">
              <uv-icon :name="item.rightIcon" size="12" :color="$colors.light"></uv-icon>
            </template>
          </uv-form-item>
        </uv-form>
      </view>
    ...
    <view
      class="px-[24rpx]  mt-[24rpx] flex flex-row items-center justify-between rounded-[32rpx]">
      <view class="w-full h-[112rpx] items-center py-[24rpx] box-border px-[32rpx] flex flex-row justify-between bg-black rounded-[32rpx] shadow-[0_0_16px_0_#266AFF_inset]">

      <text class="text-">设为默认</text>
      <uv-switch v-model="is_default" activeValue="1" inactiveValue="0" size="20" :activeColor="$colors.theme"
    ...
    <view class="box-border absolute w-full bottom-0 bg-neutral-bottomBtnBg rounded-t-[16rpx] px-[32rpx] py-[24rpx]">
      <wk-button @submit="handleSubmit" height="80rpx" fontSize="30rpx">
