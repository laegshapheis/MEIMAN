<template>
  <layout navTitle="地址管理" bgType="bg-address" :isLottie="false">
    ...
    <scroll-view scroll-y>
      <uv-radio-group v-if="addressList.length != 0" class="px-[32rpx] mt-[24rpx]" @change="change"
        v-model="is_default">
        <view v-for="item in addressList" :key="item.id" class="w-full box-border mt-[32rpx] flex flex-col justify-between bg-black p-[32rpx] rounded-[32rpx] shadow-[0_0_16px_0_#266AFF_inset]">
          <view @click="selectAddress(item)" class="flex flex-col">
            ...
            <text :class="['text-lg mt-[10rpx] font-medium', selectId == item.id ? 'text-theme' : '']">{{ item.location}} {{item.address_mask }}</text>
          </view>
          <view class="border-solid border-[#202337] border-b-0 border-l-0 border-r-0 border-t-[0.5px] my-[24rpx]"></view>
          <view class="flex flex-row items-center justify-between">
            <uv-radio shape="square" :name="item.id" :inactiveColor="$colors.secondary" :activeColor="$colors.theme"
              iconColor="#fff" :label="item.id !== is_default ? '设为默认' : '已设为默认'"
            ...
            <view>
              ...
              <view class="flex flex-row items-center">
                <view class="text-neutral-secondary text-sm" @click.stop="handleDelete(item)">
                  <image class="w-[32rpx] h-[32rpx]" src="/static/images/common/del_icon.svg"></image>
                </view>
                <view class="text-neutral-secondary text-sm ml-[24rpx]" @click.stop="handleEdit(item)">
                  <image class="w-[32rpx] h-[32rpx]" src="/static/images/common/edit_icon.svg"></image>
                </view>
              </view>
    ...
    <view class="bg-neutral-bottomBtnBg fixed w-full bottom-0 left-0 px-[32rpx] py-[24rpx] box-border">
      <wk-button @submit="addAddress" height="80rpx" fontSize="30rpx">
        新建收货地址
      </wk-button>
    </view>
