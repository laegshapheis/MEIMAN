<template>
  <layout navTitle="提前还款" bgType="bg-main1" :isLottie="false" ref="layoutRef" :refresher="true"
    @onRefresh="handleRefresh">
    <template v-slot:navRight>
      <view class="flex items-center justify-center w-[64rpx] h-[64rpx] bg-white/10 rounded-full">
        <image @click="jumpLogs" src="/static/images/user/record_icon.svg" mode="widthFix" class="w-[32rpx] h-[32rpx]">
        </image>
      </view>
    </template>
        ...
        <view class="mt-[28rpx]">
          ...
          <view class="mt-[28rpx] ">
            <wk-input maxlength="33" type="number" fontSize="38rpx" v-model="inp_val" placeholder="请输入金额"
              :bgColor="$colors.inputBg" readonly placeholder-class="text-neutral-placeholder text-2xl" />
          </view>
        </view>

        <view class="py-[16rpx] pb-[0] mt-[16rpx]">
          <wk-button height="80rpx" @submit="shen_qing_huan_kuan">申请还款</wk-button>
        </view>
      </view>
    ...
    <wk-code-input ref="keywordRef" :value="pay_password" @finish="handlePasswordComplete" />
