<template>
  <layout ref="layoutRef" :refresher="true" @onRefresh="handleRefresh" navTitle="充值记录" bgType="bg-recharge"
    ...
    <view class="px-page-x pt-page-y pb-[30rpx] box-border w-full flex flex-col">
      <view class="flex flex-row justify-around rounded-[24rpx] h-[186rpx] bg-card3">
        <view class="flex flex-col items-center justify-center">
          <text class="text-base text-white/60">充值成功({{ symbolStore.code }})</text>
          <text class="font-semibold leading-6 text-2xl mt-[16rpx] text-white">{{ chenggong }}</text>
        </view>

        <view class="flex flex-col items-center justify-center">
          <text class="text-base text-white/60">充值失败({{ symbolStore.code }})</text>
          <text class="font-semibold leading-6 text-2xl mt-[16rpx] text-white">{{ shibai }}</text>
        </view>
      </view>
      <view class="flex flex-col mt-[22rpx]">
        <view size="small" class="flex flex-col mb-[24rpx]  bg-black px-[24rpx] py-[48rpx] rounded-[32rpx] shadow-[0_0_16px_0_#266AFF_inset]" borderRadius="24rpx" v-for="item in dataList"
          :key="item.id">
          <view
            ...
            <text :class="['text-base px-[8rpx] pt-[5rpx] pb-[8rpx] rounded-[8rpx]',
              item.status == 0
                ? 'text-neutral-error '
                : item.status == -1
                  ? 'text-neutral-error'
          ...
          <view class="flex flex-col mt-[24rpx] text-neutral-regular">
            <view
              class="flex flex-row justify-between items-center text-base leading-5 pb-[16rpx] border-b border-solid border-b-neutral-divider border-x-0 border-t-0">
              ...
              <text class="text-neutral-regular">
                ...
                <template v-else>{{ item.amount }} {{ symbolStore.code }}
                </template>
              </text>
            </view>

            <view
              class="flex flex-row justify-between items-center text-base leading-5 py-[16rpx] border-b border-solid border-b-neutral-divider border-x-0 border-t-0">
              <text class="text-neutral-secondary">到账金额：</text>
              <text class="text-neutral-regular">{{ item.amount }} {{ symbolStore.code }}</text>
            </view>
