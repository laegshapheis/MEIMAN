			...
			<view class="uv-tabs__wrapper__scroll-view-wrapper">
				...
				>
					<view
						...
						:style="{
							flex: scrollable ? '' : 1,
							// borderBottom: showBottomBorder ? `${$uv.addUnit(lineHeight)} solid ${bottomBorderColor}` : 'none',
							position: 'relative',
						}"
					>
						<view
							...
							:style="[{flex: scrollable ? '' : 1},$uv.addStyle(itemStyle),
								{
									borderBottom: showBottomBorder ? `0.5px solid ${bottomBorderColor}` : 'none',
								}
							]"
						...
						<view
							...
							:style="[{
								width: $uv.addUnit(dynamicLineWidth),
...
<script>
	 ...
	 * @property {Boolean}	lineFullWidth	滑块是否铺满容器宽度，true为铺满，false为居中（默认 false ）
	 * @property {Boolean}	showBottomBorder	是否显示底部边框（默认 true ）
	 * @property {String}	bottomBorderColor	底部边框颜色（默认 '#e6e6e6' ）
	...
	export default {
		...
		computed: {
			...
			dynamicLineWidth() {
				return this.currentLineWidth || this.$uv.getPx(this.lineWidth)
			}
		},
		async mounted() {
			this.init()
		...
		methods: {
			setLineLeft() {
				...
				if (this.lineFullWidth) {
					// 新模式：滑块铺满容器宽度
					this.currentLineWidth = tabItem.rect.width;
					// 滑块从当前标签的左边界开始，铺满整个标签宽度
