<template>
  <view class="lime-dialer" :style="getStyle">
    <view class="lime-dialer__inner" :style="getDialStyle">
      ...
      <view class="lime-dialer__inner-wrap">
        <view
          class="lime-dialer__inner-item"
          v-for="(item, index) in prizeList"
          :key="index"
          :style="[getRotateAngle(index)]"
        >
          <view
            class="lime-dialer__inner-content"
    ...
    <view class="lime-dialer__pointer" :style="pointerStyle">
      ...
      <image
        ...
        src="/static/images/zhizhen.png"
        style="width: 190rpx; height: 284rpx; top: 4rpx; right: 55rpx"
        mode="widthFix"
        @tap="$emit('click')"
      />
    </view>
    ...
    getRotateAngle() {
      return (index) => {
        return {
          ...
          }deg) skewX(0deg) skewY(${360 / this.prizeList.length - 90}deg)`,
          background: `${
            this.styleOpt.prizeBgColors[
              index % this.styleOpt.prizeBgColors.length
            ]
          }`,
    ...
    getCorrectAngle() {
      return (index) => {
        return {
          ...
          }deg) skewX(0deg) rotate(${180 / this.prizeList.length}deg)`,
        };
      };
    },
    getStyle() {
...
$dialer_text_color ?= #fff
$dialer_prize_font_size ?= 16px
